...
<p style="color:#FE1764">__Empleo sin descuento jubilatorio__</p>
...
### Intensidad del trabajo de las empleadas de Servicio doméstico
...
<p style="color:#FE1764">__Cantidad de hogares en los que trabajan__</p>
```{r echo=FALSE, message=FALSE, warning=FALSE}
serv.dom %>% 
...
<p style="color:#FE1764">__Proporción sin descuento jubilatorio dentro de trabajadoras domesticas__</p>
```{r echo=FALSE, message=FALSE, warning=FALSE}
serv.dom %>% 
  summarise("Tasa de no registro" = formato_porc(sum(PONDERA[PP07H==2])/sum(PONDERA)*100)) %>% 
  kable() %>% 
...
<p style="color:#FE1764">__... vacaciones pagas?__</p>   
```{r echo=FALSE, message=FALSE, warning=FALSE}
serv.dom %>% 
  mutate(Vacaciones_pagas = case_when(PP07G1 == 1 ~ "Sí",
                                      PP07G1 == 2 ~ "No")) %>% 
  ...
  mutate(Porcentaje = formato_porc(Cantidad/sum(Cantidad)*100),
         Cantidad = formato_cantidad(Cantidad)) %>% 
  select("¿Tiene vacaciones pagas?" = Vacaciones_pagas,
         Cantidad, Porcentaje) %>% 
  kable() %>% 
  kable_styling(c("striped", "hover", "condensed", "responsive"),
                fixed_thead = T) %>% 
...
<p style="color:#FE1764">__...aguinaldo?__</p>   
```{r echo=FALSE, message=FALSE, warning=FALSE}
serv.dom %>% 
  mutate(Aguinaldo = case_when(PP07G2 == 1 ~ "Sí",
                               PP07G2 == 2 ~ "No")) %>% 
  group_by(Aguinaldo) %>% 
  summarise(n = n(),
            Cantidad = sum(PONDERA)) %>% 
  mutate(Porcentaje = formato_porc(Cantidad/sum(Cantidad)*100),
         Cantidad = formato_cantidad(Cantidad)) %>% 
  select("¿Tiene aguinaldo?" = Aguinaldo,
         Cantidad, Porcentaje) %>% 
  kable() %>% 
  kable_styling(c("striped", "hover", "condensed", "responsive"),
                fixed_thead = T) %>% 
...
<p style="color:#FE1764">__... días pagos por enfermedad?__</p>   
```{r echo=FALSE, message=FALSE, warning=FALSE}
serv.dom %>% 
  mutate(Dias_enfermedad = case_when(PP07G3 == 1 ~ "Sí",
                                     PP07G3 == 2 ~ "No")) %>% 
  ...
  mutate(Porcentaje = formato_porc(Cantidad/sum(Cantidad)*100),
         Cantidad = formato_cantidad(Cantidad)) %>% 
  select("¿Tiene días pagos por enfermedad?" = Dias_enfermedad,
         Cantidad, Porcentaje) %>% 
  kable() %>% 
  kable_styling(c("striped", "hover", "condensed", "responsive"),
                fixed_thead = T) %>% 
...
<p style="color:#FE1764">__... obra social?__</p>   
```{r echo=FALSE, message=FALSE, warning=FALSE}
serv.dom %>% 
  mutate(Obra_social = case_when(PP07G4 == 1 ~ "Sí",
                                     PP07G4 == 2 ~ "No")) %>% 
  ...
  mutate(Porcentaje = formato_porc(Cantidad/sum(Cantidad)*100),
         Cantidad = formato_cantidad(Cantidad)) %>% 
  select("¿Tiene obra social?" = Obra_social,
         Cantidad, Porcentaje) %>% 
  kable() %>% 
  kable_styling(c("striped", "hover", "condensed", "responsive"),
                fixed_thead = T) %>% 
...
<p style="color:#FE1764">__Ingreso promedio (de la ocupación principal) de las trabajadoras domésticas__</p> 
```{r echo=FALSE, message=FALSE, warning=FALSE}
serv.dom %>% 
...
<p style="color:#FE1764">__Ingreso promedio (incluyendo ocupaciones secundarias) de las trabajadoras domésticas__</p> 
```{r echo=FALSE, message=FALSE, warning=FALSE}
serv.dom %>% 
