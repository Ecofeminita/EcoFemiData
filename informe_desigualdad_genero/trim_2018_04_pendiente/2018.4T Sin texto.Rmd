...
```{r}
# Cambiar inlines, mejorar con pull()
# Chequear formatos de pesos y porcentajes
# colores[1]
# Agregar botones de descarga de tablas (y ver si se pueden reordenar las columnas)
```
...
inline_1 <- tabla1 %>% 
  ...
  transmute(diferencia = round((Varones-Mujeres)*100,-1)) # transmute() conserva solo las variables que creo
...
inline_3 <- tabla2m$`de 14 a 29 años`[tabla2m$Indicadores == "Tasa Desocupación"]

inline_4 <- tabla2v$`de 14 a 29 años`[tabla2v$Indicadores == "Tasa Desocupación"]
...
inline_5 <- tabla4$`Sin descuento jubilatorio`[tabla4$Sexo == "Mujeres"]
inline_6 <- tabla4$`Sin descuento jubilatorio`[tabla4$Sexo == "Varones"]
```
...
inline_7 <- formato_porc((tabla5$proporcion[tabla5$JERARQUIA %in% c("Dirección o Jefes") & tabla5$Sexo == "Varones"]) * 100)
inline_8 <- formato_porc((tabla5$proporcion[tabla5$JERARQUIA %in% c("Dirección o Jefes") & tabla5$Sexo == "Mujeres"]) * 100)
```
...
COPIAR LO MISMO ACÁ PERO CON ASALARIADES Y AGREGAR TASA DE NO REGISTRO
...
inline_9 <- tabla7$`Brecha`[tabla7$Población == "Perceptores de ingresos"]
inline_10 <- tabla7$`Brecha`[tabla7$Población == "Ocupadas/os"]

inline_11 <- tabla7$`Mujeres`[tabla7$Población == "Ocupadas/os"]
inline_12 <- tabla7$`Varones`[tabla7$Población == "Ocupadas/os"]

inline_13 <- tabla7$`Brecha`[tabla7$Población == "Asalariadas/os sin desc. jubil."]
```
...
inline_17 <- formato_pesos(tabla5.1$IOP_mensual[tabla5.1$Sexo == "Varones" & tabla5.1$CALIFICACION == "No Calificados"], dec = 0)

inline_18 <- formato_pesos(tabla5.1$IOP_mensual[tabla5.1$Sexo == "Mujeres" & tabla5.1$CALIFICACION == "No Calificados"], dec = 0)
...
ggplot(tabla8_graf, 
       aes(x = Mujeres, xend = Varones, 
           y = CALIFICACION, 
  spread(., Sexo, IOP_mensual) %>% 
  mutate(brecha = percent((Varones-Mujeres)/Varones),
         x = (Varones+Mujeres)/2)
...
ggplot(tabla13.1_graf, aes(x = as.numeric(DECCFR), Prop, fill = Sexo, group= Sexo, label = paste0(formato_porc(Prop*100), "%"))) +
  geom_col(position = "dodge") +
...
ggplot(tabla12.1_graf, aes(x = as.numeric(DECINDR), Prop, fill = Sexo, group= Sexo, label = paste0(formato_porc(Prop*100), "%"))) +
  geom_col(position = "dodge")+
  geom_text(position = position_dodge(width = .9), vjust =-.1, size = 2.5)+
  theme_minimal()+
  theme(axis.text.y = element_blank(),
