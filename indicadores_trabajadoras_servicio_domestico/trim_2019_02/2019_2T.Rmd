...
El presente informe tiene como objetivo visibilizar las condiciones de trabajo, ingresos, y principales características de las trabajadoras que prestan servicio doméstico en hogares particulares. Para ello se presentan diversos indicadores, cuadros y gráficos de elaboración propia. Para realizar los cálculos, se utilizan bases de microdatos publicados por la Encuesta Permanente de Hogares (EPH - INDEC - Argentina). Esta información refiere a los 31 aglomerados urbanos que releva la encuesta, en este caso durante el `r date`. El documento se realizó íntegramente utilizando el lenguaje de programación R y todo el código se encuentra disponible para su descarga.
<br><br>
Sabemos que puede volverse un poco técnico por momentos, pero seguramente desde [Economía Femini(s)ta](http://economiafeminita.com/) realicemos también otras notas de carácter divulgatorio que podrás encontrar en nuestra página. Obviamente, estás invitade a utilizar estos datos en notas, trabajos, infografías, etc. citando nuestro trabajo^[Shokida, Natsumi S. (05/03/2020) Las trabajadoras de servicio doméstico en Argentina. 2do trimestre de 2019. _Economía Femini(s)ta_. Recuperado de https://ecofeminita.github.io/EcoFemiData/indicadores_trabajadoras_servicio_domestico/trim_2019_02/2019_2T.nb.html]. Además, si te gusta nuestro trabajo podés apoyarnos potenciando nuestros proyectos, haciéndote socie del [#ClubEcofeminita o realizando una donación](https://donaronline.org/economia-femini-s-ta/club-ecofeminita).
...
<p style="color:#FE1764">__Proporcion de mujeres y de varones en el sector__</p>
```{r echo=FALSE, message=FALSE, warning=FALSE}
serv.dom %>% 
...
<p style="color:#FE1764">__Proporción de la ocupación dentro de las ocupadas mujeres__</p>
```{r echo=FALSE, message=FALSE, warning=FALSE}
base_individual %>% 
  filter(ESTADO == 1,
         Sexo == "Mujeres") %>% 
...
<p style="color:#FE1764">__Proporción de la ocupación dentro de las asalariadas mujeres__</p>
```{r echo=FALSE, message=FALSE, warning=FALSE}
base_individual %>% 
  filter(ESTADO == 1,
         CAT_OCUP == 3,
...
### Características {.tabset .tabset-fade .tabset-pills}

En esta sección se presentan algunas métricas respecto a la distribución etaria, el lugar de nacimiento y características educativas de las trabajadoras del sector de servicio doméstico. 
