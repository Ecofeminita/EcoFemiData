...
serv.dom %>% 
  filter(PP3E_TOT > 0, # Horas trabajadas positivas
         PP3E_TOT != 999,
         PP3F_TOT != 999) %>% 
  filter(PP04B2 > 0) %>% #C asos con información sobre cantidad de hogares en los que trabajan
  mutate(cantidad_hogares = case_when(PP04B2 == 1 ~ "1",
                                      PP04B2 == 2 ~ "2",
                                      PP04B2 >= 3 ~ "3 o más")) %>% 
  group_by(cantidad_hogares) %>% 
...
tabla6a <- serv.dom %>% 
  summarise("Ingreso promedio" = weighted.mean(IPCF, as.numeric(PONDIH)), #saqué los NA pero no estoy segura de que esté bien 
