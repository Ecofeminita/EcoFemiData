...
subtitle: "1er trimestre de 2021"
author: "#EcoFemiData, equipo de datos de Economía Feminita - Agosto de 2021"
output: 
  html_notebook:
    theme: flatly
urlcolor: #E5616E
...
base_individual <- eph::get_microdata(year = 2021, trimester = 1, type =  "individual") # <-----------------
base_hogar <- eph::get_microdata(year = 2021, trimester = 1, type =  "hogar")           # <-----------------
```
...
<br><br>
En cuanto a la cantidad de hogares en que se desempeñan, un `r inline_22`% trabaja habitualmente en una sola casa, por lo que al mismo tiempo casi un `r inline_23`% de ellas suele trabajar en más de una vivienda. __Mientras que las que son contratadas por un sólo hogar trabajaron cerca de `r inline_24` horas semanales (jornada part-time), aquellas que trabajan en más de una casa trabajaron en promedio menos de `r inline_25` horas por semana en la ocupación principal, acercándose a las `r inline_26` horas semanales con las ocupaciones secundarias__.
...
tabla1a <- serv.dom %>% 
  ...
  summarise("Ingreso promedio" = weighted.mean(P21, PONDIIO),
            "50% cobra menos de" = spatstat::weighted.median(P21, PONDIIO)) %>% 
...
tabla2a <- serv.dom %>% 
  ...
  summarise("Ingreso promedio" = weighted.mean(P21/(PP3E_TOT * 30 / 7), PONDIIO),
            "50% cobra menos de" = (spatstat::weighted.median(P21/(PP3E_TOT * 30 / 7), PONDIIO))) %>% 
...
tabla3a <- serv.dom %>% 
  summarise("Ingreso promedio" = weighted.mean(P47T - T_VI, PONDII),
            "50% cobra menos de" = spatstat::weighted.median(P47T - T_VI, PONDII)) %>% 
...
tabla4a <- serv.dom %>% 
  summarise("Ingreso promedio" = weighted.mean(P47T, PONDII),
            "50% cobra menos de" = spatstat::weighted.median(P47T, PONDII)) %>% 
...
tabla5a <- serv.dom %>% 
  summarise("Ingreso promedio" = weighted.mean(ITF, PONDIH),
            "50% cobra menos de" = spatstat::weighted.median(ITF, PONDIH)) %>% 
...
tabla6a <- serv.dom %>% 
  summarise("Ingreso promedio" = weighted.mean(IPCF, PONDIH, na.rm = TRUE), #saqué los NA pero no estoy segura de que esté bien 
            "50% cobra menos de" = spatstat::weighted.median(IPCF, PONDIH)) %>% 
...
```{r message=FALSE, warning=FALSE, include=FALSE}
rm(tabla1, tabla2, tabla3, tabla4, tabla5, tabla6,tabla1a, tabla2a, tabla3a, tabla4a, tabla5a, tabla6a)
```

_Nota: Las remuneraciones mensuales fueron calculadas sin incluir a las trabajadoreas que no trabajaron en el período de referencia pero de todas formas percibieron un salario. Se trata de `r inline_xx` trabajadoras, de acuerdo a `r inline_xx` casos muestrales. De realizar el cálculo contemplandolas el salario promedio seria de `r inline_xx`. Antes de la pandemia estos casos eran minoritarios y no modificaban el ingreso promedio._  
...
<br><br>
Al mismo tiempo, mientras que __un `r inline_39`% no es el primer sostén económico del hogar, un `r inline_40`% sí lo es__. En ambos casos, la mayoría de ellas realizan las tareas domésticas en su hogar. Esto es así porque __un `r inline_41`% de quienes trabajan de forma remunerada realizando trabajo doméstico también están a cargo de la realización de este trabajo en sus hogares__ de manera no-asalariada, y por tanto no remunerada.
