...
date <- as.character("2do trimestre de 2024")                        # <-----------------
...
# NS: para evitar notación científica
options(scipen = 999)
...
base_individual <- eph::get_microdata(year = 2024, trimester = 2, type =  "individual") # <-----------------
base_hogar <- eph::get_microdata(year = 2024, trimester = 2, type =  "hogar")           # <-----------------
```
...
tabla1a <- serv.dom %>%
  filter(PP3E_TOT > 0, # Horas trabajadas positivas
         PP3E_TOT != 999) |> 
  summarise("Ingreso promedio" = weighted.mean(as.numeric(P21), as.numeric(PONDIIO)),
            "50% cobra menos de" = weighted.median(as.integer(P21), as.integer(PONDIIO))) %>% 
...
tabla2a <- serv.dom %>% 
  ...
  summarise("Ingreso promedio" = weighted.mean(P21/(PP3E_TOT * 30 / 7), PONDIIO),
            "50% cobra menos de" = weighted.median(P21/(PP3E_TOT * 30 / 7), PONDIIO)) %>% 
...
tabla3a <- serv.dom %>% 
  summarise("Ingreso promedio" = weighted.mean(P47T - T_VI, PONDII/sum(PONDII)),
            "50% cobra menos de" = weighted.median(P47T - T_VI, PONDII)) %>% 
...
tabla4a <- serv.dom %>% 
  summarise("Ingreso promedio" = weighted.mean(P47T, PONDII/sum(PONDII)),
            "50% cobra menos de" = weighted.median(P47T, PONDII)) %>% 
...
tabla5a <- serv.dom %>% 
  summarise("Ingreso promedio" = weighted.mean(ITF, as.numeric(PONDIH)),
            "50% cobra menos de" = weighted.median(ITF, PONDIH)) %>% 
...
tabla6a <- serv.dom %>% 
  summarise("Ingreso promedio" = weighted.mean(IPCF, as.numeric(PONDIH)), 
            "50% cobra menos de" = weighted.median(IPCF, PONDIH)) %>% 
...
tabla1ab <- serv.dom %>% 
  ...
  summarise("Ingreso promedio" = weighted.mean(P21, PONDIIO),
            "50% cobra menos de" = weighted.median(P21, PONDIIO)) %>% 
  mutate(Ingresos =  "Mensuales, de la ocupación principal")
...
serv.dom %>% 
  filter(PP3E_TOT > 0, # Horas trabajadas positivas
         PP3E_TOT != 999) %>% 
  group_by(Descuento_jubilatorio) %>% 
  summarise(Ingresos = formato_pesos(weighted.mean(as.numeric(P21), as.numeric(PONDIIO))),
