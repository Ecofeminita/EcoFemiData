...
datos <- read.csv("Fuentes/precios-gestion-menstrual-2023-03-01.csv", 
                  header = TRUE, sep = ",", dec = ".", 
...
Contamos con `r length(unique(marcas$Marca))` marcas de productos, aunque con 13 de ellas se completa el 90% del total de productos. Las primeras 4 marcas concentran más de la mitad de las observaciones. Nuevamente aquí hay que tener en cuenta que no se trata exactamente de una "concentración de mercado".
...
Más del l 80% de los productos vienen en paquetes de 8 o 16 unidades. Luego seguiremos analizando esta variable en particular, porque nos interesa para calcular los precios unitarios de los productos, y hay valores (como "1.0 un") que son raros y, en caso de tratarse de errores, podrían arrastrar problemas hacia el cálculo de esos precios.
...
En particular, hay casos que fueron informados como de 30 o 54 unidades y en realidad eran de 8 (tiene sentido que este último sea el verdadero valor), un caso que fue informado como de 27 unidades y en realidad era de 16, y un caso que fue informado como de 12 unidades y en realidad era de 20. 
...
Vale la pena destacar que estos errores corresponden a las regiones Gran Buenos Aires, Pampeana y Patagonia, mayormente concentrados en GBA, y en 3 marcas diferentes.
...
```{r echo=TRUE}
saveRDS(datos, file = "Fuentes/precios-gestion-menstrual-2023-03-19-limpio.RDS")
```