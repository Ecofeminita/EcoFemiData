...
En esta ocasión, por primera vez se encontró entre los registros obtenidos algunos con la palabra "copa" dentro de la descripción, que se encontraban camuflados dentro del categoría tampón. Para poder continuar correctamente con el preprocesamiento, creamos la categoría "copa", para poder distinguir realmente estos productos de las toallitas y los tampones, luego la filtramos por el momento. En total hay `r total_observaciones` observaciones en total, de las cuales filtramos `r total_observaciones-total_sin_copas` correspondientes a la nueva categoría "copa", arribando así a `r total_sin_copas` observaciones. Sobre esta muestra trabajaremos a lo largo del resto del documento.
...
Contamos con `r length(unique(marcas$Marca))` marcas de productos, aunque con 11 de ellas se completa el 90% del total de productos. Las primeras 3 marcas concentran más de la mitad de las observaciones. Nuevamente aquí hay que tener en cuenta que no se trata exactamente de una "concentración de mercado".
...
```
Por esta vez, filtramos de los datos los precios mayores a 1.000 pesos, basicamente porque hay casos (outliers que de hecho vamos a tener que corregir) que superan ese monto y "rompen" la escala del gráfico. Para una comprensión más acabada de este efecto, veamos qué sucede si quitamos estos valores que hasta superan los 1.000 pesos:
...
Como puede verse arriba, hay en total `r cants_iguales %>% filter(igual == FALSE) %>% pull(n)` casos en que la información no coincide, lo que representa el `r cants_iguales %>% filter(igual == FALSE) %>% pull(porc)` de la información (mejoró considerablemente respecto del último relevamiento), y sólo `r cants_iguales %>% filter(is.na(igual)) %>% pull(n)` casos en que no había información sobre la cantidad de unidades en la descripción del artículo.
