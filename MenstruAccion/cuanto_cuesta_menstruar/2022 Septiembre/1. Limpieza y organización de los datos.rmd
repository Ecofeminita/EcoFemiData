...
author: "Datos de Septiembre 2022"
output:
  html_document:
    toc: no
    toc_depth: '5'
...
En el marco del cálculo del costo de menstruar en Argentina, llevado a cabo desde la campaña #MenstruAcción y EcoFeminita, en el presente documento de trabajo se encuentra la exploración, organización y limpieza de los datos obtenidos mediante el [#MenstruScrapper](https://github.com/nportas/menstruscrapper), desarrollado por Nayla Portas de [LAS] de sistemas. El #MenstruScrapper fue ejecutado el día 7 de Septiembre de 2022 sobre la página Precios Claros, sumándose esta información a la obtenida anteriormente en Septiembre y Marzo de 2019, Marzo y Septiembre de 2020, Marzo y Septiembre de 2021 y Marzo de 2022. En esta instancia, se obtuvieron 38.939 observaciones de tampones y toallitas en total.

+ Respecto de los tampones: Se obtuvieron 6.257 precios.
+ Respecto de las toallitas: Se obtuvieron 32.682 precios.
...
datos <- read.csv("Fuentes/precios-gestion-menstrual-2022-09-07.csv", 
                  header = TRUE, sep = ",", dec = ".", 
...
En esta ocasión, por primera vez se encontró entre los registros obtenidos algunos con la palabra "copa" dentro de la descripción, que se encontraban camuflados dentro del categoría tampón. Para poder continuar correctamente con el preprocesamiento, creamos la categoría "copa", para poder distinguir realmente estos productos de las toallitas y los tampones, luego la filtramos por el momento. En total hay 40.826 observaciones en total, de las cuales filtramos 1.887 correspondientes a la nueva categoría "copa", arribando así a 38.939 observaciones. Sobre esta muestra trabajaremos a lo largo del resto del documento.
...
Contamos con 25 marcas de productos, aunque con 11 de ellas se completa el 90% del total de productos. Las primeras 3 marcas concentran más de la mitad de las observaciones. Nuevamente aquí hay que tener en cuenta que no se trata exactamente de una "concentración de mercado".
...
El 80% de los productos vienen en paquetes de 8 o 16 unidades. Luego seguiremos analizando esta variable en particular, porque nos interesa para calcular los precios unitarios de los productos, y hay valores (como "1.0 un") que son raros y, en caso de tratarse de errores, podrían arrastrar problemas hacia el cálculo de esos precios.
...
Si bien dos tercios de las observaciones se concentran en Buenos Aires y CABA, es importante saber que se cuenta con información de todas las provincias del país, y que incluso en el peor de los casos (por ejemplo, 64 observaciones en Formosa o Catamarca) se trata de una cantidad pasible de proveernos estimaciones útiles.
...
comparo_unidades
```
En particular, hay casos que fueron informados como de 1 o 16 unidades y en realidad eran de 8 (tiene sentido que este último sea el verdadero valor), casos que fueron informados como de 15 o 27 unidades y en realidad eran de 16, o casos que fueron informados como de 54 unidade y en realidad eran de 8, etc. Como puede verse a continuación, hay en total 743 casos en que la información no coincide, lo que representa el 0.34% de la información (mejoró considerablemente respecto del último relevamiento), y sólo 3 casos en que no había información sobre la cantidad de unidades en la descripción del artículo.
...
Vale la pena destacar que estos errores corresponden a las regiones Gran Buenos Aires, Noroeste, Pampeana y Patagonia, mayormente concentrados en GBA, y en 4 marcas diferentes.
...
Esta vez, dado que el porcentaje de aquellos casos en que no coinciden `unidades` y `unidades_regex`, junto a los casos en que `unidades_regex` figura como `NA`, alcanzan solamente un 0.34%, se prescindirá de ellos. 
...
__Se descartan 1169 casos__ y ahora contamos con `r cant(nrow(datos))` observaciones. También podemos deshacernos de variables que no vamos a utilizar en el ejercicio de estimación.
...
```{r echo=TRUE}
saveRDS(datos, file = "Fuentes/precios-gestion-menstrual-2022-09-16-limpio.RDS")
```