...
Contamos con 27 marcas de productos, aunque con 12 de ellas se completa el 90% del total de productos. Las primeras 3 marcas concentran más de la mitad de las observaciones. Nuevamente aquí hay que tener en cuenta que no se trata exactamente de una "concentración de mercado".
...
Si bien más de la mitad de las observaciones se concentran en Buenos Aires y CABA, es importante saber que se cuenta con información de todas las provincias del país, y que incluso en el peor de los casos (por ejemplo, 50 observaciones en Catamarca) se trata de una cantidad pasible de proveernos estimaciones útiles.
...
Esta vez, dado que el porcentaje de aquellos casos en que no coinciden `unidades` y `unidades_regex`, junto a los casos en que `unidades_regex` figura como `NA`, alcanzan solamente un 2.3%, se prescindirá de ellos. 
...
__Se descartan 743 casos__ y ahora contamos con `r cant(nrow(datos))` observaciones. También podemos deshacernos de variables que no vamos a utilizar en el ejercicio de estimación.
...
```{r echo=TRUE}
saveRDS(datos, file = "Fuentes/precios-gestion-menstrual-2022-03-13-limpio.RDS")
```