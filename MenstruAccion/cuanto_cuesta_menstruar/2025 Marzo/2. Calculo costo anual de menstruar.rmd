...
datos <- readRDS("Fuentes/precios-gestion-menstrual-2025-03-10-limpio.RDS")
...
```{r}
precios_pgm <- rbind(precios_pgm, c("01/03/2025", round(valor_toallitas,2), round(valor_tampones,2)))

precios_pgm <- precios_pgm %>% 
  mutate(Mes = as.Date(Mes, format = "%d/%m/%Y"),  # Especifica el formato de fecha
...
ggplot(base100bis, aes(x = Mes, y = Indice, group = Categoria, color = Categoria)) +
  ...
  expand_limits(x = as.POSIXct(c("2019-03-01", "2024-09-01"))) + 
  labs(title = "Inflación de Toallitas y Tampones vs. Salud y Nivel General del IPC-INDEC",
       subtitle = "Marzo 2019 - Marzo 2025", #<------------------------------------------------- CAMBIAR
       x = "",
       y = "Indice de precios",
