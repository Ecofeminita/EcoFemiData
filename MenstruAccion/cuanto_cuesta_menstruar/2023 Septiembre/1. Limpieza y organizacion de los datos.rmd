...
author: "Datos de Septiembre 2023"
output:
  html_document:
    toc: no
    toc_depth: '5'
...
datos <- read.csv("Fuentes/precios-gestion-menstrual-2023-09-15.csv", 
                  header = TRUE, sep = ",", dec = ".", 
...
Contamos con `r length(unique(marcas$Marca))` marcas de productos, aunque con 11 de ellas se completa el 90% del total de productos. Las primeras 4 marcas concentran más de la mitad de las observaciones. Nuevamente aquí hay que tener en cuenta que no se trata exactamente de una "concentración de mercado".
...
En particular, hay casos que fueron informados como de 54 unidades y en realidad eran de 8 (tiene sentido que este último sea el verdadero valor), un caso que fue informado como de 27 unidades y en realidad era de 16, y un caso que fue informado como de 12 unidades y en realidad era de 20. 
...
Vale la pena destacar que estos errores corresponden a las regiones Gran Buenos Aires y Pampeana, mayormente concentrados en GBA, y en 2 marcas diferentes.
...
```{r echo=TRUE}
saveRDS(datos, file = "Fuentes/precios-gestion-menstrual-2023-09-19-limpio.RDS")
```