...
subtitle: Datos de Septiembre 2023
output:
  html_document:
    toc: no
    toc_depth: '5'
...
datos <- readRDS("Fuentes/precios-gestion-menstrual-2023-09-19-limpio.RDS")
...
```{r}
precios_pgm <- rbind(precios_pgm, c("2023-09-01", round(valor_toallitas,2), round(valor_tampones,2)))
...
ggplot(base100bis, aes(x = Mes, y = Indice, group = Categoria, color = Categoria)) +
  ...
  expand_limits(x = as.POSIXct(c("2019-03-01", "2023-09-01"))) + 
  labs(title = "Inflación de Toallitas y Tampones vs. Salud y Nivel General del IPC-INDEC",
       subtitle = "Marzo 2019 - Septiembre 2023", #<------------------------------------------------- CAMBIAR
       x = "",
       y = "Indice de precios",
       caption = "Fuente: #MenstruAcción y #EcoFemiData (EcoFeminita)") +
  scale_x_datetime(breaks = seq(as.POSIXct('2019-03-01'), by='6 months', length = 10), #<------------------------ CAMBIAR
                   labels = c("03-2019", "09-2019", "03-2020", "09-2020", "03-2021", "09-2021", "03-2022", "09-2022", "03-2023", "09-2023")) #<------------- CAMBIAR
```
