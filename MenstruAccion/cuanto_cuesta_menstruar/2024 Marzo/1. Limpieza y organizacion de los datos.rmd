...
datos <- read.csv("Fuentes/precios-gestion-menstrual-2024-03-05.csv", 
                  header = TRUE, sep = ",", dec = ".", 
...
Contamos con `r length(unique(marcas$Marca))` marcas de productos, aunque con 8 de ellas se completa casi el 90% del total de productos. Las primeras 3 marcas concentran más de la mitad de las observaciones. Nuevamente aquí hay que tener en cuenta que no se trata exactamente de una "concentración de mercado".
...
datos <- datos %>% 
  filter(Precio.de.lista < 3000) #saco los outliers mayores a 1000 porque son demasiado altos. 
...
```{r echo=TRUE}
saveRDS(datos, file = "Fuentes/precios-gestion-menstrual-2024-03-05-limpio.RDS")
```